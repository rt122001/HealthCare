...
def get_disease(symptomlist,buckets):
	...
	final_csv = score_dropped[:3].to_csv("/Library/WebServer/Documents/htdocs/hospital3/healthcareproject/a-priori/final.csv",index=False)
	...
	rand = random.randrange(0,len(one))
	final.append(one[rand])
	two = list(set(lis[1])-set(lis[2]) - set(lis[0]))
	rand = random.randrange(0,len(two))
	final.append(two[rand])
	three = list(set(lis[2])-set(lis[0]) - set(lis[1]))
	rand = random.randrange(0,len(three))
	final.append(three[rand])

	final_list = []
	for i in range(len(final)):
		final_list.append([final[i],disease[i]])
	
	asd = pd.DataFrame(data=final_list,columns=['Disease','Symptom']).to_csv("/Library/WebServer/Documents/htdocs/hospital3/healthcareproject/a-priori/mutually_exclusive.csv")
